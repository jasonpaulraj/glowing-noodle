...
<div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center py-4 sm:pt-0">
    ...
    <h1><a href="{{ URL::route('positionbox.manage', ['position_box' => $data['id']]) }}" class="ml-1 underline">Manage</a></h1>
    ...
    <table class="table table-bordered" width="50%" border="1" cellpadding="3" cellspacing="0">
        <tbody>
                ...
                <tr>
                    <!-- Repeat columns -->
                    @for($column=0;$column<$data['box_columns'];$column++) 

                        <!-- 
                        If the looped row & looped column matches the 
                        stipulated position for the sentence content, display the sentence 
                        -->
                        @if(
                            isset($sentenceRow,$sentenceColumn) && 
                            ($row == $sentenceRow) && 
                            ($column == $sentenceColumn))

                            @if(isset($content['position_box_text']['sentence']))
                            @php($sentence = $content['position_box_text']['sentence'])
                            <td width="50" height="50">{{$sentence}}</td>
                            @endif
                        @else 
